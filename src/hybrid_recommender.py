...
class HybridRecommender:
    ...
    def recommend(self, 
        ...
        all_restaurants = pd.concat([
            cf_recommendations[['restaurant_id']] if len(cf_recommendations) > 0 else pd.DataFrame(columns=['restaurant_id']),
        ...
        all_restaurants = all_restaurants.merge(
            self.restaurant_features[[
                'restaurant_id', 'name', 'cuisine_type', 'avg_rating', 
                'price_range', 'avg_delivery_time', 'popularity_score',
        ...
        contextual_score = self._compute_contextual_score(all_restaurants, context, user_location)
        ...
        if not has_history:
            ...
            adjusted_weights = {
                ...
                'content_based': self.weights['collaborative_filtering'] + self.weights['content_based'],
        ...
        if user_location:
            all_restaurants = self._filter_by_distance(all_restaurants, user_location, max_distance_km=10)
        ...
        result_columns = [
            'rank', 'restaurant_id', 'name', 'cuisine_type', 'avg_rating',
            'price_range', 'avg_delivery_time', 'final_score',
            'cf_score', 'content_score', 'contextual_score',
            'avg_order_value'
        ]
        
        return recommendations[result_columns].reset_index(drop=True)
    
    def _compute_contextual_score(self, 
                                   restaurants: pd.DataFrame,
                                   context: Optional[Dict],
                                   user_location: Optional[Tuple[float, float]]) -> np.ndarray:
        """
    ...
    def _calculate_distances(self, 
                            restaurants: pd.DataFrame,
                            user_location: Tuple[float, float]) -> np.ndarray:
        """
        ...
        lon_diff = (restaurants['location_lon'].values - user_lon) * 111 * np.cos(np.radians(user_lat))
...
if __name__ == "__main__":
    ...
    print(recommendations[['rank', 'name', 'cuisine_type', 'avg_rating', 'final_score']])
