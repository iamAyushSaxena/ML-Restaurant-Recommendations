...
class ExplainabilityEngine:
    ...
    def explain(self, 
        ...
        ].iloc[0] if user_id in self.user_features['user_id'].values else None
        ...
        if user_id in self.cf_model.interaction_matrix.index:
            ...
            if similar_users:
                ...
                similar_users_who_ordered = [
                    uid for uid in similar_user_ids
        ...
        if context:
            ...
            if time_of_day and restaurant['cuisine_type'] in time_cuisine_match.get(time_of_day, []):
                meal_name = time_of_day.replace('_', ' ').title()
                reasons.append({
                    'type': 'contextual',
                    'weight': 'medium',
                    'text': EXPLANATION_TEMPLATES['contextual'].format(meal_time=meal_name)
                })
        ...
        return {
            ...
            'supporting_reasons': [r['text'] for r in reasons[1:4]],  # Top 3 supporting reasons
