...
class FeatureEngineer:
    ...
    def __init__(self, users_df: pd.DataFrame, restaurants_df: pd.DataFrame, 
    ...
    def create_user_features(self) -> pd.DataFrame:
        ...
        user_order_stats = self.orders_df.groupby('user_id').agg({
            'order_id': 'count',
            'order_value': ['mean', 'std', 'sum'],
            'delivery_time': 'mean',
            'user_rating': 'mean',
            'restaurant_id': 'nunique',  # Unique restaurants ordered from
            'cuisine_type': lambda x: x.mode()[0] if not x.empty else None  # Most frequent cuisine
        }).reset_index()
        
        user_order_stats.columns = [
            'user_id', 'order_count', 'avg_order_value', 'order_value_std',
            'total_spent', 'avg_delivery_time', 'avg_rating_given',
            'unique_restaurants', 'most_ordered_cuisine'
        ]
        ...
        user_features['unique_restaurants'] = user_features['unique_restaurants'].fillna(0)
        
        # === Cuisine diversity score ===
        # How diverse are user's cuisine choices?
        cuisine_diversity = self.orders_df.groupby('user_id')['cuisine_type'].apply(
        ...
        user_features['cuisine_diversity'] = user_features['cuisine_diversity'].fillna(0)
        
        # === Recency features ===
        # Days since first order
        user_first_order = self.orders_df.groupby('user_id')['order_timestamp'].min().reset_index()
        ...
        user_features.loc[user_features['order_count'] >= 20, 'user_segment'] = 'regular'
        user_features.loc[user_features['order_count'] >= 50, 'user_segment'] = 'power_user'
        
        # === Price preference score (1-4 scale) ===
        price_map = {'low': 1.5, 'medium': 2.5, 'high': 3.5}
        user_features['price_preference_score'] = user_features['price_sensitivity'].map(price_map)
        
        # === Encoding categorical variables ===
        # Label encode dietary preference
        dietary_map = {'veg': 1, 'non_veg': 2, 'vegan': 3, 'no_preference': 0}
        user_features['dietary_preference_encoded'] = user_features['dietary_preference'].map(dietary_map)
        
        # Label encode favorite cuisine
        le_cuisine = LabelEncoder()
        user_features['favorite_cuisine_encoded'] = le_cuisine.fit_transform(user_features['favorite_cuisine'])
    ...
    def create_restaurant_features(self) -> pd.DataFrame:
        ...
        restaurant_order_stats = self.orders_df.groupby('restaurant_id').agg({
            'order_id': 'count',
        ...
        restaurant_order_stats.columns = [
            'restaurant_id', 'total_orders', 'avg_order_value', 'total_revenue',
            'avg_user_rating', 'unique_customers'
        ]
        ...
        restaurant_features['total_orders'] = restaurant_features['total_orders'].fillna(0)
        restaurant_features['unique_customers'] = restaurant_features['unique_customers'].fillna(0)
        ...
        repeat_customers = self.orders_df.groupby(['restaurant_id', 'user_id']).size().reset_index(name='order_count')
        repeat_customers = repeat_customers[repeat_customers['order_count'] > 1]
        repeat_stats = repeat_customers.groupby('restaurant_id').size().reset_index(name='repeat_customers')
        
        restaurant_features = restaurant_features.merge(repeat_stats, on='restaurant_id', how='left')
        restaurant_features['repeat_customers'] = restaurant_features['repeat_customers'].fillna(0)
        
        restaurant_features['retention_rate'] = (
            restaurant_features['repeat_customers'] / 
        ...
        restaurant_features['delivery_efficiency'] = restaurant_features['delivery_efficiency'].clip(0, 1)
        ...
        restaurant_features['is_veg_only_int'] = restaurant_features['is_veg_only'].astype(int)
    ...
    def create_user_restaurant_matrix(self) -> pd.DataFrame:
        ...
        # Frequency score
        user_restaurant_orders = interactions.groupby(
        ...
        user_restaurant_orders.columns = [
            'user_id', 'restaurant_id', 'order_count', 'avg_rating', 'last_order'
        ]
        
        # Recency score (exponential decay)
        days_since_last = (datetime.now() - user_restaurant_orders['last_order']).dt.days
        user_restaurant_orders['recency_score'] = np.exp(-days_since_last / 30)  # 30-day half-life
        
        # Combined interaction score
        user_restaurant_orders['interaction_score'] = (
            0.4 * np.log1p(user_restaurant_orders['order_count']) +  # Log scale for frequency
        ...
        print(f"✅ Created interaction matrix: {interaction_matrix.shape[0]} users × {interaction_matrix.shape[1]} restaurants")
        print(f"   Sparsity: {(interaction_matrix == 0).sum().sum() / (interaction_matrix.shape[0] * interaction_matrix.shape[1]) * 100:.1f}%")
        
        self.interaction_matrix = interaction_matrix
        return interaction_matrix
    
    def calculate_distance_features(self, user_location: Tuple[float, float]) -> pd.DataFrame:
        """
        ...
        restaurant_coords = self.restaurant_features[['restaurant_id', 'location_lat', 'location_lon']].copy()
        
        # Simplified distance calculation (not exact haversine)
        # For demo purposes, using Euclidean distance with lat/lon scaling
        lat_diff = (restaurant_coords['location_lat'] - user_lat) * 111  # 1 degree lat ≈ 111 km
        lon_diff = (restaurant_coords['location_lon'] - user_lon) * 111 * np.cos(np.radians(user_lat))
    ...
    def create_contextual_features(self, context: Dict) -> Dict:
        ...
        contextual_features['time_encoding'] = time_map.get(time_of_day, [0, 1, 0, 0, 0])
        
        # Day of week (binary: weekday vs weekend)
        contextual_features['is_weekend'] = 1 if context.get('day_of_week') == 'weekend' else 0
        
        # Weather influence on cuisine preference
        weather = context.get('weather', 'clear')
        weather_cuisine_boost = {
            'rainy': {'Street Food': 0.5, 'Fast Food': 1.5, 'Chinese': 1.3},  # Comfort food
            'hot': {'Beverages': 1.8, 'Desserts': 1.5, 'South Indian': 0.8},  # Cool items
            'clear': {}  # No adjustment
        }
        contextual_features['weather_cuisine_boost'] = weather_cuisine_boost.get(weather, {})
    ...
    def save_features(self):
        ...
        if self.user_features is not None:
            self.user_features.to_csv(PROCESSED_DATA_DIR / 'user_features.csv', index=False)
            print(f"💾 Saved user features to {PROCESSED_DATA_DIR / 'user_features.csv'}")
        
        if self.restaurant_features is not None:
            self.restaurant_features.to_csv(PROCESSED_DATA_DIR / 'restaurant_features.csv', index=False)
            print(f"💾 Saved restaurant features to {PROCESSED_DATA_DIR / 'restaurant_features.csv'}")
        
        if self.interaction_matrix is not None:
            self.interaction_matrix.to_csv(PROCESSED_DATA_DIR / 'interaction_matrix.csv')
            print(f"💾 Saved interaction matrix to {PROCESSED_DATA_DIR / 'interaction_matrix.csv'}")
