...
class RecommendationEvaluator:
    ...
    def __init__(self, test_orders: pd.DataFrame, restaurant_features: pd.DataFrame):
        """
        Args:
    ...
    def precision_at_k(
        self, recommendations: List[str], actual_orders: List[str], k: int = 10
    ) -> float:
        """
    ...
    def novelty_score(
        self, recommendations: List[str], user_order_history: List[str]
    ) -> float:
        """
    ...
    def coverage_score(self, all_recommendations: List[List[str]]) -> float:
        """
        ...
        coverage = (
            len(all_recommended) / total_restaurants if total_restaurants > 0 else 0
        )

        return coverage

    def evaluate_model(
        self, recommender, test_users: List[str], k_values: List[int] = [5, 10, 20]
    ) -> Dict:
        """
        ...
        for user_id in test_users:
            ...
            actual_orders = (
                self.test_orders[self.test_orders["user_id"] == user_id][
                    "restaurant_id"
                ]
                .unique()
                .tolist()
...
if __name__ == "__main__":
    ...
    test_users = test_orders["user_id"].unique()[
        :100
    ]  # Sample 100 users for faster evaluation

    results = evaluator.evaluate_model(
        hybrid_model, test_users.tolist(), k_values=[5, 10, 20]
    )
