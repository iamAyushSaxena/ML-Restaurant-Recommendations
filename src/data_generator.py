...
class RestaurantDataGenerator:
    ...
    def generate_restaurants(self) -> pd.DataFrame:
        ...
        name_prefixes = ['Tasty', 'Spicy', 'Royal', 'Golden', 'Fresh', 
                        'Express', 'Paradise', 'Corner', 'Deluxe', 'Classic']
        name_suffixes = ['Kitchen', 'Bites', 'Hub', 'Palace', 'Point', 
                        'Cafe', 'Restaurant', 'Grill', 'House', 'Eatery']
        ...
        reviews = np.random.lognormal(mean=5, sigma=1.5, size=RESTAURANT_BASE).astype(int)
        reviews = np.clip(reviews, 10, 10000)
        
        # Price range (1-4)
        price_ranges = np.random.choice([1, 2, 3, 4], size=RESTAURANT_BASE, 
        ...
        # Veg/Non-veg
        is_veg_only = np.random.choice([True, False], size=RESTAURANT_BASE, 
                                       p=[0.30, 0.70])
    ...
    def generate_users(self) -> pd.DataFrame:
        ...
        total_orders = np.random.lognormal(mean=1.5, sigma=1.2, size=USER_BASE).astype(int)
        ...
        days_since_last_order = np.random.exponential(scale=10, size=USER_BASE).astype(int)
    ...
    def generate_orders(self) -> pd.DataFrame:
        ...
        for idx, user in self.users_df.iterrows():
            ...
            if dietary_pref == 'veg':
                ...
                candidate_restaurants = candidate_restaurants[
                    (candidate_restaurants['is_veg_only'] == True) &
                    (candidate_restaurants['cuisine_type'].isin(['Healthy', 'Salads']))
                ]
            ...
            candidate_restaurants['selection_prob'] /= candidate_restaurants['selection_prob'].sum()
            
            # Select restaurants for this user's orders
            # Users tend to repeat orders from liked restaurants
            unique_restaurants_count = max(3, int(n_orders * 0.6))  # 60% unique restaurants
            ...
            for _ in range(n_orders):
                ...
                order_value = base_value * price_multiplier * np.random.uniform(0.8, 1.2)
                ...
                delivery_time = restaurant['avg_delivery_time'] + np.random.randint(-5, 10)
        ...
        # Sort by timestamp
        self.orders_df = self.orders_df.sort_values('order_timestamp').reset_index(drop=True)
    ...
    def save_data(self):
        ...
        if self.restaurants_df is not None:
            self.restaurants_df.to_csv(SYNTHETIC_DATA_DIR / 'restaurants.csv', index=False)
    ...
    def generate_summary_stats(self):
        ...
        if self.orders_df is not None:
            ...
            print(f"   Average Order Value: ₹{self.orders_df['order_value'].mean():.2f}")
            print(f"   Average Delivery Time: {self.orders_df['delivery_time'].mean():.0f} min")
            print(f"   Date Range: {self.orders_df['order_timestamp'].min().date()} to {self.orders_df['order_timestamp'].max().date()}")
