...
class ColdStartHandler:
    ...
    def onboarding_recommend(self, 
        ...
        if favorite_cuisines:
            for cuisine in favorite_cuisines:
                candidates.loc[
                    candidates['cuisine_type'] == cuisine,
                    'cold_start_score'
        ...
        candidates['cold_start_score'] += (
            0.30 * (candidates['popularity_score'] / max_popularity)
        )
        ...
        if len(recommendations) < n_recommendations:
            ...
            for rest in remaining:
                if len(recommendations) >= n_recommendations:
                    break
                if rest['restaurant_id'] not in [r['restaurant_id'] for r in recommendations]:
        ...
        result_columns = [
            ...
            'price_range', 'avg_delivery_time', 'cold_start_score',
            'avg_order_value'
        ]
    ...
    def popular_recommend(self, n_recommendations: int = 10) -> pd.DataFrame:
        ...
        popular = self.restaurant_features.sort_values(
            by=['popularity_score', 'avg_rating', 'total_reviews'],
            ascending=False
        ).head(n_recommendations).copy()
    ...
    def similar_user_cold_start(self,
                               user_profile: Dict,
...
if __name__ == "__main__":
    ...
    print(recommendations[['rank', 'name', 'cuisine_type', 'avg_rating', 'cold_start_score']])
