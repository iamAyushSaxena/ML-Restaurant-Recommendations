...
class CollaborativeFilteringRecommender:
    ...
    def fit(self):
        ...
        self.fitted = True
        print(f"✅ Collaborative Filtering model fitted (Sparse Matrix Shape: {self.sparse_interaction_matrix.shape})")
    ...
    def get_similar_users(self, user_id: str, k: int = 10) -> List[Tuple[str, float]]:
        ...
        similarities = cosine_similarity(user_vector, self.sparse_interaction_matrix).flatten()
    ...
    def recommend(self, user_id: str, n_recommendations: int = 10, 
                  exclude_already_ordered: bool = True) -> pd.DataFrame:
        """
        ...
        if exclude_already_ordered:
            restaurant_scores = {
                rid: score for rid, score in restaurant_scores.items()
                if rid not in already_ordered
            }
        
        # Convert to DataFrame and sort
        recommendations = pd.DataFrame(
            list(restaurant_scores.items()),
            columns=['restaurant_id', 'cf_score']
        ).sort_values('cf_score', ascending=False).head(n_recommendations)
    ...
    def load_model(cls, filepath: str = None):
        ...
        if model.sparse_interaction_matrix is None and model.fitted:
            model.sparse_interaction_matrix = csr_matrix(model.interaction_matrix.values)
